...
$(function(){
  $('#site_header').load('data/header_en.php',function(){
    //异步请求完成(即页头加载完成)后的回调
  ...
  success: function(data){
    ...
    $.each(data, function(i, p){
      ...
      <tr>
          ...
          <td><a href="${p.did}">delete</a></td>
      </tr>  
      `;
      sum += parseFloat(p.price*p.count); //计算页面所有商品的总计
    });
    $('#cart tbody').html(html);
    $("#cart_footer span").html(sum);
  },
  error: function(){
...
$('#cart tbody').on('click', '.subBtn', function(e){
  ...
  if(v<=0){
    input.val=1;
    alert("The quantity cannot be less")
  }
  //3:保存回input
  input.val(v);
  //4:发送ajax请求更新数据库
...
$("#addOrder").click(function(){
	location.href = 'addorder_en.html';
});